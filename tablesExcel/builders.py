...
from .handlers import PersonalTableRender, GroupTimesheetRender, GroupKPIRender


def build_personal_table_result(worker, month):
...
def build_month_timesheet(month: Month):
    query = get_query_for_timesheet(month)
    mean = month.get_means()
    ts = GroupTimesheetRender(month)
...
def build_month_kpi(month: Month):
    query = get_query_kpi(month)

    report = GroupKPIRender(month)
